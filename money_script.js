...
function applyTestModeMenuState(mode) {
  ...
  if (modeHint) {
    if (isSpeedTestMode(normalizedMode)) {
      modeHint.textContent = 'Показывается только общий секундомер теста.';
    } else if (isUntimedTestMode(normalizedMode)) {
      modeHint.textContent = 'Во время теста время не показывается, но учитывается в статистике.';
    } else {
      modeHint.textContent = 'Для каждого вопроса действует отдельный таймер.';
    }
  }
}
      ...
      <div class="history-item">
        <div class="history-item-head">
          <div class="history-summary">
            ...
            <div class="history-meta">
              ...
              <span>⚙️ ${escapeHtml(entry.modeLabel || getTestModeLabel(entry.testMode))}</span>
              ${durationMetaHtml}
        ...
        <div class="history-details hidden" data-id="${escapeHtml(entry.id)}">
          <div class="history-detail-grid">
            ...
            <div><b>Режим:</b> ${escapeHtml(entry.modeLabel || getTestModeLabel(entry.testMode))}</div>
